...
import type { UpdateRecipeForm } from '#shared/types/recipe/updateRecipeForm.type'
...
const state = reactive<Partial<UpdateRecipeForm>>({
  name: '',
  description: '',
  emoji: '🍽️',
  time: 0,
...
async function onSubmit(event: FormSubmitEvent<UpdateRecipeForm>) {
  isSubmitting.value = true

  try {
    await recipeService.updateRecipe(props.recipeId, event.data)

    await Promise.all([
      invalidateQuery(QUERY_KEYS.RECIPE_INDEX),
      invalidateQuery(QUERY_KEYS.RECIPE_COUNT),
      invalidateQuery(`${QUERY_KEYS.RECIPE_INDEX}-${props.recipeId}`),
    ])
